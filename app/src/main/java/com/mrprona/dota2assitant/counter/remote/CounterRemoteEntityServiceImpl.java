...
import com.mrprona.dota2assitant.counter.api.TruepickerReccomends;
...
public class CounterRemoteEntityServiceImpl extends BaseRemoteServiceImpl implements CounterRemoteEntityService {
    ...
    public Pair<List<Counter>, String> getCounters(Context context, List<Integer> allies, List<Integer> enemies,
        ...
        //url.append(level);
        Pair<TruepickerReccomends, String> response = basicRequestSend(context, url.toString(), TruepickerReccomends.class);
        List<Counter> counters = new ArrayList<Counter>();
        if (response.first != null) {
            String firstTeam = response.first.getRecommendsForRadiant();
